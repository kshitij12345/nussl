...
class AudioSignal(object):
    ...
    **Sample Rate**
    ...
    def audio_data(self, value):
        ...
        elif not isinstance(value, (np.ndarray, torch.Tensor)):
        ...
        if value.ndim < 2:
            value = torch.expand_dims(value, axis=constants.CHAN_INDEX)
    ...
    def stft_data(self, value):
        ...
        # if not torch.iscomplexobj(value):
    ...
    def freq_vector(self):
        ...
        return torch.linspace(
            ...
            steps=self.stft_data.shape[constants.STFT_VERT_INDEX])
    ...
    def load_audio_from_file(self, input_file_path, offset=0, duration=None, new_sample_rate=None):
        ...
        # Read data as torch.Tensor!
        self.audio_data = torch.tensor(audio_input, device='cuda:0')
    ...
    def stft(self, window_length=None, hop_length=None, window_type=None, overwrite=True):
        ...
        for chan in self.get_channels():
            assert type(chan) == torch.Tensor
            # import pickle
            # with open('p.pkl', 'wb') as f:
            #     print("Hope", hop_length)
            #     pickle.dump({'chan':chan, 'fs':self.sample_rate, 'window':window,
            #                 'nperseg': window_length, 'noverlap': window_length-hop_length}, f)
            # _, _, _stft = scipy.signal.stft(
            #     chan, fs=self.sample_rate, window=window,
            #     nperseg=window_length, noverlap=window_length - hop_length)

            _stft = torch.stft(chan, window_length, hop_length=hop_length, win_length=window_length, window=torch.tensor(window, device=chan.device), return_complex=True)
            stft_data.append(_stft)
            # assert type(_stft) == torch.Tensor

        arr_class = type(chan)
        if arr_class == torch.Tensor:
            # print(torch.stack(stft_data).shape)
            stft_data = torch.stack(stft_data).squeeze(1).permute((1, 2, 0))
        else:
            stft_data = arr_class(stft_data).permute((1, 2, 0))
    ...
    def istft(self, window_length=None, hop_length=None, window_type=None, overwrite=True,
        ...
        for stft in self.get_stft_channels():
            ...
            _signal = torch.istft(stft, window_length, hop_length=hop_length, win_length=window_length, window=torch.tensor(window, device=stft.device))

            signals.append(np.array(_signal.cpu()))
    ...
    def apply_mask(self, mask, overwrite=False):
        ...
        magnitude, phase = torch.abs(self.stft_data), torch.angle(self.stft_data)
        masked_abs = magnitude * mask.mask
        masked_stft = masked_abs * torch.exp(1j * phase)
    ...
    def ipd_ild_features(self, ch_one=0, ch_two=1):
        ...
        ild = torch.abs(stft_ch_one) / (torch.abs(stft_ch_two) + 1e-4)
        ild = 20 * torch.log10(ild + 1e-8)

        frequencies = self.freq_vector
        ipd = torch.angle(stft_ch_two * torch.conj(stft_ch_one))
